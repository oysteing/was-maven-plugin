...
public class AppManager {
	...
	private static Logger LOGGER = Logger.getLogger(AppManager.class.getName());
	...
	public boolean isStarted(String appName) {
		ObjectName query;
		try {
			query = new ObjectName ("WebSphere:type=Application,name=echoear,*");
	...
	public void startApplication(String appName) {
		if (isStarted(appName)) {
			LOGGER.fine("Application " + appName + " is already started. Doing nothing.");
			return;
		}
		LOGGER.fine("Starting application " + appName);
		am.startApplication(appName);
		LOGGER.fine("Application " + appName + " started");
	}
